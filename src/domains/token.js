...
/**
 * Signed integer fluff as BigInt. Same strictness as toFluff, but the
 * sign is allowed: internal journal legs are double-entry, so debit
 * legs are negative. Public amounts must still go through toFluff.
 */
function toSignedFluff(value, field = "amountFluff") {
  if (typeof value === "bigint") return value;
  if (!Number.isSafeInteger(value)) {
    throw new TypeError(`${field} must be a safe integer (fluff)`);
  }
  return BigInt(value);
}
  ...
  _commit({ action, legs, idempotencyKey, actor = "sim", memo = null, linkedJournalId = null, settle = false }) {
    ...
    const normLegs = legs.map((leg, i) => {
      ...
      return {
        ...
        amount: toSignedFluff(leg.amount, `legs[${i}].amount`),
      };
    });
