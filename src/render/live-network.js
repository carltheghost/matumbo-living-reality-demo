...
export function mountLiveNetworkConsole({ documentRoot = globalThis.document, client = createLiveNetworkClient() } = {}) {
  ...
  open.textContent = "LIVE NETWORK";
  open.id = "matumbo-live-open";
  Object.assign(open.style, {
    position:"fixed", right:"18px", bottom:"18px", zIndex:"179", padding:"10px 13px",
    borderRadius:"999px", border:"1px solid rgba(125,232,255,.28)",
    background:"rgba(3,12,18,.82)", color:"#dff7ff", cursor:"pointer",
  });

  const panel = documentRoot.createElement("section");
  panel.id = "matumbo-live-console";
  panel.hidden = true;
  panel.setAttribute("aria-label", "Live network");
  panel.innerHTML = `
  ...
  const status = (text, kind = "") => {
    ...
    el.textContent = text;
    el.className = "ml-status " + kind;
  };

  function replaceTextList(target, rows, emptyText) {
    target.replaceChildren();
    if (!rows.length) {
      const empty = documentRoot.createElement("div");
      empty.className = "ml-small";
  ...
  const renderMembers = (items = []) => {
    const rows = items.map(m => {
      ...
      row.textContent = `${m.user?.displayName || "Anonymous"} · ${m.state || "unknown"} `;
      const seen = documentRoot.createElement("span");
      seen.className = "ml-small";
      seen.textContent = m.seenAt || "";
      row.appendChild(seen);
      return row;
    });
    replaceTextList(q("[data-members]"), rows, "No active members.");
  };

  async function members() {
    try { renderMembers((await client.members(room)).members); } catch {}
  }

  async function markets() {
    try {
      q("[data-market-status]").textContent = "Refreshing…";
      const data = await client.markets(q("[data-provider]").value, 50);
      q("[data-market-status]").textContent = `${data.records.length} live observations · ${data.retrievedAt}`;
      const rows = data.records.slice(0, 30).map(r => {
        const card = documentRoot.createElement("div");
        const heading = documentRoot.createElement("div");
        const provider = documentRoot.createElement("b");
        provider.textContent = r.provider || "provider";
        heading.append(provider, documentRoot.createTextNode(` · ${r.title || "Untitled market"}`));
        const detail = documentRoot.createElement("div");
        detail.className = "ml-small";
        const probability = r.probability == null ? "—" : `${Math.round(r.probability * 100)}%`;
        detail.textContent = `probability ${probability} · ${r.status || "unknown"}`;
        card.append(heading, detail);
        return card;
      });
      replaceTextList(q("[data-market-list]"), rows, "No records returned.");
    } catch (e) {
      q("[data-market-status]").textContent = e.message;
    }
  }

  q("[data-api]").value = client.base || "";
  q("[data-health]").onclick = async () => {
    try {
      const h = await fetch(client.base + "/api/health").then(r => r.json());
      status(`API online · ${h.storage} · accounts ${h.accounts} · multiplayer ${h.multiplayer}`, "ok");
    } catch (e) { status(e.message, "err"); }
  };
  q("[data-markets]").onclick = markets;
  q("[data-provider]").onchange = markets;
  q("[data-api]").onchange = e => {
    client.configure(e.target.value);
    q("[data-auth]").textContent = "Not signed in.";
    status(client.base ? "API configured. Sign in again if needed." : "API not configured.");
  };
  q("[data-register]").onclick = async () => {
    try {
      await client.register(q("[data-user]").value, q("[data-pass]").value, q("[data-name]").value);
      status("Account created. Log in to continue.", "ok");
    } catch (e) { status(e.message, "err"); }
  };
  q("[data-login]").onclick = async () => {
    try {
      const d = await client.login(q("[data-user]").value, q("[data-pass]").value);
      q("[data-auth]").textContent = `Signed in as ${d.user.displayName}`;
      status("Signed in.", "ok");
    } catch (e) { status(e.message, "err"); }
  };
  q("[data-logout]").onclick = async () => {
  ...
  q("[data-delete]").onclick = async () => {
    if (!globalThis.confirm?.("Delete this account and its collected app events?")) return;
    try {
      await client.deleteAccount();
      q("[data-auth]").textContent = "Account deleted.";
      stream?.close();
      stream = null;
      status("Account and collected app events deleted.", "ok");
    } catch (e) { status(e.message, "err"); }
  };
  q("[data-join]").onclick = async () => {
    room = q("[data-room]").value.trim() || "lobby";
    await members();
    stream?.close();
    stream = client.stream(room, sequence, event => {
      sequence = Math.max(sequence, event.sequence || 0);
      const line = documentRoot.createElement("div");
      line.textContent = event.type + " · " + (event.user?.displayName || "system");
      q("[data-feed]").prepend(line);
      members();
    });
    try {
      await client.presence(room, { state: "online" });
      status(`Joined ${room}`, "ok");
    } catch (e) { status(e.message, "err"); }
  };
  q("[data-send]").onclick = async () => {
    const input = q("[data-message]");
    if (!input.value.trim()) return;
    if (input.value.length > 1000) return status("Message is limited to 1000 characters.", "err");
    try {
      await client.postEvent(room, "message", { text: input.value.trim() });
      input.value = "";
    } catch (e) { status(e.message, "err"); }
  };
  q("[data-export]").onclick = async () => {
    try {
      ...
      a.download = "matumbo-my-data.json";
      a.click();
  ...
  q("[data-share]").onclick = async () => {
    ...
    const link = api ? location.origin + location.pathname + "?api=" + encodeURIComponent(api) : location.href;
    try {
      await navigator.clipboard.writeText(link);
      status("Share link copied.", "ok");
    } catch { status(link); }
  };
  q("[data-close]").onclick = () => { panel.hidden = true; open.hidden = false; };
  open.onclick = () => { panel.hidden = false; open.hidden = true; };

  return Object.freeze({
    open: () => { panel.hidden = false; open.hidden = true; },
    close: () => { panel.hidden = true; open.hidden = false; },
    client,
  });
}
