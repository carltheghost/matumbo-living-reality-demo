/**
 * Packet 233 — feature block worlds.
 *
...
import {CONNECTED_CITY_DISTRICTS} from '../domains/connected-city.js';
...
export function createFeatureWorlds({THREE,renderer,scene,camera,controls,onNavigate,onFrame,reducedMotion=false,features=[],domains,onEnter,onExit}={}){
  ...
  // --- Tiny shared glass recipe (same language as reality-assembly-scene.js) ---
  ...
  const glass=(tint='#153954',opacity=C.maxGlassOpacity)=>{
    const material=track(new THREE.MeshStandardMaterial({color:tint,transparent:true,opacity,depthWrite:false,roughness:.15,metalness:.1}),materials);
    return material;
  };
  const edgeMaterial=color=>track(new THREE.LineBasicMaterial({color,transparent:true,opacity:.8}),materials);
  ...
  styleEl.textContent=`
    .feature-world{position:fixed;inset:0;pointer-events:none;z-index:30;font-family:inherit;}
    .feature-world [data-back]{position:absolute;top:14px;left:14px;pointer-events:auto;background:rgba(10,26,40,.72);border:1px solid rgba(125,212,255,.5);color:#cfe9ff;border-radius:10px;padding:9px 14px;font-size:14px;cursor:pointer;backdrop-filter:blur(6px);}
  ...
  function buildWorld(featureId,content){
    ...
    content.forEach((cubeDef,index)=>{
      ...
      label.onclick=()=>{selectCube(cubeDef.id);};
      label.ondblclick=()=>{selectCube(cubeDef.id);performCubeAction(nodeById(cubeDef.id).cube.action,nodeById(cubeDef.id).cube);};
  ...
  function destroy(){
    ...
    canvas?.removeEventListener('dblclick',activate);
  }
  ...
  const activate=event=>{
    if(!machine.getSnapshot().active)return;
    const id=locate(event);
    if(id){const node=nodeById(id);if(node){selectCube(id);performCubeAction(node.cube.action,node.cube);}}
  };
  const canvas=renderer?.domElement;
  canvas?.addEventListener('pointermove',hover);
  canvas?.addEventListener('pointerdown',press);
  canvas?.addEventListener('dblclick',activate);
