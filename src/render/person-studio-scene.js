...
import {updateTumboChibiRig} from './tumbo-chibi-rig.js?v=20260920-p238';
import {buildTumboFluffyRig} from './tumbo-fluffy-rig.js?v=20260920-p238';

/** PERSON Ω avatar: the fluffy Tumbo mascot IS the avatar. (Packet 238)
 *
 ...
 *
 * Packet 238 hardening (locked 5): the rig build is deferred to first
 * Person Studio open — createPersonStudio never builds the 76–104 fur
 * shells at boot; update() mounts on the first visible frame and no-ops
 * avatar work until then. The build is wrapped in try/catch: a throw logs
 * a warning and leaves a rig-less studio (loud, never a dead app), and the
 * fingerprint promise resolves null so the owner falls back to the model
 * hash — fail-closed re-approval, zero data loss. */
export function buildPersonStudioScene({THREE,parent,targets=[],compact=false,avatarTextureUrl='assets/avatar/fluffy-body-template.webp'}) {
  ...
  // Declared ahead of mountRig: a late (deferred) mount must wear the
  // currently selected outfit's muff tint, not always the default.
  let muffColor=STUDIO_OUTFITS[0].muffs??'#f5f2ea';
  let rig=null,rigDecalMats=[],rigBuildFailed=false;
  function mountRig(){
    rigDecalMats=[];
    const built=buildTumboFluffyRig(THREE,{
      seed:0,
      muffColor:muffColor??'#5f646c',
  ...
  let identityBytes=null,identityDigest=null,identityResolver=null;
  const identityPromise=new Promise(resolve=>{identityResolver=resolve;});
  function computeIdentityFingerprint(){
    identityGeometry.length=0;
    ...
    identityBytes=new TextEncoder().encode(JSON.stringify({model:STUDIO_MODEL,geometry:identityGeometry}));
    identityResolver?.(identityBytes);
  }
  // Boot-path hardening (locked 5): the rig build never runs at module or
  // createPersonStudio time. The first visible update() mounts it; a throw
  // logs a warning, flags rigBuildFailed (loud in the snapshot), and leaves
  // a rig-less studio — never a dead app. On failure the fingerprint
  // resolves null and the owner falls back to the STUDIO_MODEL hash.
  function mountRigSafe(){
    try{
      rig=mountRig();
      collectPickMeshes();
      computeIdentityFingerprint();
    }catch(error){
      console.warn('[person-studio-scene] fluffy rig build failed; studio continues without an avatar.',error);
      rig=null;rigDecalMats=[];rigBuildFailed=true;
      identityResolver?.(null);
    }
  }
  async function fingerprint(cryptoRoot=globalThis.crypto){
    const bytes=await identityPromise;
    if(!bytes)return null;
    if(!identityDigest)identityDigest=cryptoRoot.subtle.digest('SHA-256',bytes).then(digestBytes=>[...new Uint8Array(digestBytes)].map(v=>v.toString(16).padStart(2,'0')).join(''));
  ...
  // Everything structural floats: the lens-ring and the set pieces bob
  // gently in lens space (frozen under reduced motion). The avatar's own
  // bob/sway/glow/walk follow the fluffy rig update — the rig bobs itself,
  // so there is no avatar entry here (keeping both would double-bob).
  ...
  function apply(snapshot){
    ...
    // hoodie in the reference. Stored always; applied to the rig when built
    // (apply runs before the deferred mount on cold boot).
    muffColor=clothing.muffs??'#5f646c';
    if(rig)rig.materials.cushion.color.set(muffColor);
  ...
  function update(dt,time,pose={joints:{}},reducedMotion=false){
    ...
    // Deferred mount (locked 5): the first visible frame builds the rig;
    // avatar work below no-ops until it exists. Set dressing animates
    // regardless of rig state.
    if(!rig&&!rigBuildFailed)mountRigSafe();
    if(rig){
      ...
      // The owner's samplePose() decays toward neutral ~2s after release, so
      // this overlay eases back to rest on its own — no fighting with the
      // poseHeld/moveRig channel, which feeds the same avatar.motion buffer.
  ...
  function getSnapshot(){return {source:'person-studio-scene',modelId:STUDIO_MODEL.id,visible:layer.visible,outfitId:outfit,roomId:room,companionForm:form,jointNames:rig?Object.keys(rig.joints):[],meshCount:avatarPickMeshes.length+geometries.size,geometryOnly:false,referenceImagesUsedAsTextures:false,rigVisible:!!rig,rigBuildFailed,referenceLook:'tumbo-fluffy-v1',earmuffColor:muffColor,tumboDecalPresent:rig?.hasFaceDecal??false,avatarHologramPresent:false,avatarHologramUrl:avatarTextureUrl,avatarHologramTint:avatarHologramTint,greetKind:getGreetKind(),avatarOffset:{x:avatarTarget.x,z:avatarTarget.z},avatarPickMeshCount:avatarPickMeshes.length};}
  ...
  function refreshRigFace(){
    // Deferred build: the stored URL lands on the rig at first mount.
    if(!rig)return;
    ...
    try{
      ...
      // Keep the previous rig rather than going avatar-less mid-session.
      console.warn('[person-studio-scene] rig face refresh failed; keeping the previous rig.',error);
      rig=old;rigDecalMats=oldDecals;
      return;
    }
  ...
  function destroy(){
    ...
    if(rig){try{rig.dispose?.();}catch{/* fluffy rig owns its geometries/materials */}}
    rigDecalMats.forEach(material=>{try{material.map?.dispose?.();}catch{}try{material.dispose?.();}catch{}});
    layer.removeFromParent();
  }
  return {layer,avatar,get joints(){return rig?rig.joints:null;},apply,update,getSnapshot,destroy,fingerprint,avatarHologram:null,avatarPickMeshes,greet,getGreetKind,setAvatarOffset,setAvatarFace,setHologramTint,resolve:object=>object?.userData?.personStudioAction??null};
}
