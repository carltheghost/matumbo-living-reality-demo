  ...
  return {open,close,get active(){return active;},getSnapshot:snapshot,resolve:spatial.resolve,selectObject(object){const id=spatial.resolve(object);if(!id)return false;select(id);toggle();return true;},
    update(dt,time){
      ...
      if(lod==='world-block'){
        ...
        const badgePlacement=placeAssemblyLabel({
          ndcX:screenPoint.x,ndcY:screenPoint.y,ndcZ:screenPoint.z,
          viewportWidth:innerWidth,viewportHeight:innerHeight,safeTop:70,safeBottom:innerHeight-70,
        });
        if(badgePlacement.visible){
          worldLabel.style.left=badgePlacement.x+'px';
          worldLabel.style.top=badgePlacement.y+'px';
        }
      }else{
