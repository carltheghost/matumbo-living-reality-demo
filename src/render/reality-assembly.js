...
import {createFeatureWorlds} from './feature-worlds.js?v=20260920-p240';
...
export function createRealityAssembly({THREE,renderer,scene,camera,controls,world,targets,features,onNavigate,onFrame,readFeature=()=>null,environmentTexture=null,reducedMotion=false}){
  ...
  const worlds=createFeatureWorlds({THREE,renderer,scene,camera,controls,onNavigate,onFrame,reducedMotion,features:ordered,
    onEnter:()=>{spatial.layer.visible=false;focusTarget=null;focusPosition=null;
      // The assembly node labels project the hidden layer's cubes — freeze
      // them out of the way while inside a world (update() re-projects them
      // on exit). World chrome paints above the assembly UI (z-200 > z-105).
      for(const [,label] of labels)label.hidden=true;},
    onExit:()=>{spatial.layer.visible=true;overview();}});
  // studio<->world visibility invariant (Packet 240): studio.open() exits any
  // active feature world first. person-studio.js is owned by another packet,
  // so the wrap lives here at the integration seam; worlds.exit() is a no-op
  // when no world is active.
  try{
    const studio=typeof window!=='undefined'?window.__TUMBO_PERSON_STUDIO__:null;
    if(studio&&typeof studio.open==='function'&&!studio.__tumboWorldExitWrapped){
      const rawOpen=studio.open;
      studio.open=function(...args){try{worlds.exit();}catch{}return rawOpen.apply(studio,args);};
      studio.__tumboWorldExitWrapped=true;
    }
  }catch{}
  ...
  const release=guard(event=>{
    ...
    if(event.type==='pointerup'&&previous.objectId&&Math.hypot(event.clientX-previous.x,event.clientY-previous.y)<7){
      select(previous.objectId);
      if(event.pointerType!=='mouse')assemblyTap.tap(previous.objectId,event);
    }
  });
  ...
  // Touch parity for the travel gesture (Packet 240): the native dblclick
  // above is the deliberate mouse path (the law names double-click); the
  // shared 350ms/28px detector covers touch/pen, for which dblclick does not
  // fire reliably. Same gesture, view-scoped destination: assembly
  // double-tap enters the feature's glass world (block-world double-tap
  // dives inside the cube instead).
  const assemblyTap=createDoubleTapDetector({
    onSingleTap:()=>{},
    onDoubleTap:event=>{
      if(!active)return;
      const hit=locate(event),id=spatial.resolve(hit?.object);
      if(id){select(id);worlds.enter(id);}
    },
  });
