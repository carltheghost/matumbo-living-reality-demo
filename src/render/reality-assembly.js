...
export function createRealityAssembly({THREE,renderer,scene,camera,controls,world,targets,features,onNavigate,onFrame,readFeature=()=>null,environmentTexture=null,reducedMotion=false}){
  ...
  <div class="assembly-world-label" data-world-label hidden>maTumbo · Reality Lens Ω</div>
  ...
  return {open,close,get active(){return active;},getSnapshot:snapshot,resolve:spatial.resolve,selectObject(object){const id=spatial.resolve(object);if(!id)return false;select(id);toggle();return true;},
    update(dt,time){
      ...
      if(lod==='world-block'){
        ...
        screenPoint.set(14,8,0).project(camera);
        const badgePlacement=placeAssemblyLabel({ndcX:screenPoint.x,ndcY:screenPoint.y,ndcZ:screenPoint.z,viewportWidth:innerWidth,viewportHeight:innerHeight,safeTop:70,safeBottom:innerHeight-70});
        if(badgePlacement.visible){worldLabel.style.left=badgePlacement.x+'px';worldLabel.style.top=badgePlacement.y+'px';worldLabel.style.transform='translate(-50%,-50%)';}
      }
      else{
