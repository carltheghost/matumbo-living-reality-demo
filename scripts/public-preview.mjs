...
export async function createPreviewServer(root) {
  ...
  async function scan() {
    ...
    return {project:'maTumbo Living Reality', revision:hash.digest('hex'), updatedAt:new Date(updated).toISOString(), publicFiles:count,
      ...
      implemented:['Centered draggable feature panels and equal-axis cube sigils', 'Source-linked and personal non-monetary single/pool authoring with parent/child contracts, explicit local entries, browser persistence, normal view and interactive cube view', 'Playable local chess Arena with legal move validation, turn state, captures, checkmate detection, animated 3D character pieces, orbit/zoom camera and accessible board view', 'Shared-scene VR/AR entry and target selection, hardware verification pending', 'Opt-in local camera/mic preview, audio-only, mute and stop; no calling backend', 'Approved-avatar domain foundation: immutable identity, versioned wardrobe, motion isolation and snapshot restore; likeness renderer pending'],
