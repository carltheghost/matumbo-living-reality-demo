...
import {
  ...
  isExpired,
  revoke
} from '../src/domains/portal-sessions/opaque-handle.js';

import {
  createSession
} from '../src/domains/portal-sessions/session.js';

import {
  registerAdapter,
  resolve
} from '../src/domains/portal-sessions/adapter-registry.js';

import {
  checkSession
} from '../src/domains/portal-sessions/health.js';
...
test('handle opacity: no credential-like fields or values', () => {
  ...
  const forbiddenKeys =
    /token|secret|password|credential|authorization|access.?key|refresh.?key/i;

  assert.equal(
    Object.keys(handle).some((key) => forbiddenKeys.test(key)),
    false
  );

  assert.equal(
    Object.values(handle).some(
      (value) => typeof value === 'string' && forbiddenKeys.test(value)
    ),
    false
  );
...
test('health statuses cover healthy, stale, expired, revoked, dead, and offline', () => {
  ...
  assert.equal(checkSession(offline, 104).status, 'dead');
});